...
const new_expense= async (req,res)=>{
    ...
    try {
        ...
        if(Number(recurring)==1){
           ...
           const sql1="insert into recurringExpenses (description,category,amount,due_date) values(?,?,?,?)";

           const [result]= await db.promise().query(sql1,[description, category, amount, due_date]);
...
const delete_expense= async (req,res)=>{
    ...
    try {
        ...
        const [results]= await db.promise().query("select user_id,month(date_created) as month,year(date_created) as year,amount from expenses where exp_id=?", [id]);
        ...
        //update monthly summary
        await db.promise().query("update monthly_summary set expense = greatest(expense - ?,0) where user_id=? and year=? and month= ? ",[ results[0].amount, results[0].user_id, results[0].year, results[0].month]);
...
const update_expense= async (req,res)=>{
    ...
    try {
        ...
        const [rows]=await db.promise().query("select amount,month(date_created) as month,year(date_created) as year, user_id from expenses where exp_id=?",[expense_id]);
        ...
        await db.promise().query(sql,[ description, category,amount, status,expense_id]);
        
        //update monthly summary
        await db.promise().query("update monthly_summary set expense = greatest(expense + ?, 0) where year=? and month=? and user_id=?",[new_amount,year,month,user_id]);
